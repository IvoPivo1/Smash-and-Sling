class GameOverScreen implements IScreen {
  ...
  private restartLevel() {
    const lvl = game.selectedLevel && game.selectedLevel >= 1 ? game.selectedLevel : 1;

    if (!game.selectedBirds || game.selectedBirds.length === 0) {
      game.currentScreen = new BirdSelectScreen();
      return;
    }

    game.currentScreen = new LevelFactory().createLevel(lvl);
  }
